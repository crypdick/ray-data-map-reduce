...
class CollectList(AggregateFn):
    

    def __init__(self, input_key: str, output_key: str = "items"):
        # The aggregator logic below is row-based: accumulate_row processes
...
def make_list_aggregator(input_key: str, output_key: str = "items"):
    """
    Creates a custom accumulator-based aggregator that collects grouped values
...
def extract_bigrams(item: dict):
    """
    Take a single line of text and yield bigrams (pairs of consecutive words).
    For each bigram, emit a record with a 1 count, e.g. {"bigram": ("the", "cat"), "count": 1}.
    """
    line = item["item"]
    tokens = re.findall(r"\w+", line.lower())
    for i in range(len(tokens) - 1):
        # gotcha: returning a tuple(w1, w2) will not work, because Ray Data will conver this
        # to an array. Then, the groupby will fail with an error like:
        # `The truth value of an array with more than one element is ambiguous.`
        # instead, we have to return a string with a special delimiter.
        yield {"bigram": " ".join([tokens[i], tokens[i + 1]]), "count": 1}
...
def main():
    ...
    # Example small text corpus: a few short lines.
    text_lines = [
